...
float CTransactFee::GetCommission(SymbolKind StockSymbol)
{
	...
	if ( (CMainFrame::m_pDoc)->m_sharesInformation.Lookup(StockSymbol.m_chSymbol, pDat,StockSymbol.m_nSymbolKind ) )     //检测该股票是否已记录在内存模板中
	{
		switch(pDat->kind) 
		{
		case 0:
			...
			break;
		case 1://沪A
			if( StockSymbol.m_chSymbol[0]=='5')//基金
