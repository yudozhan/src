...
BCMenu::BCMenu()
{
	...
	m_bitmapBackground=RGB(255,0,255); //gray
...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	COLORREF m_clrBack=GetSysColor(COLOR_WINDOW);//COLOR_WINDOW
	...
	if(state & MF_SEPARATOR){
		...
		BOOL checkflag=FALSE;
		COLORREF crText = GetSysColor(COLOR_MENUTEXT);
		COLORREF crSelect = GetSysColor(RGB(255,0,0));
