#define MAXDATANUM	( ALLTIME * 60 / TIMERDELAY )		//(分钟*60秒/间隔)
...
//	DATAMM	dataMm[MAXDATANUM];
	float        volume_per_num;//每分钟的实时笔数
	float        volume_per[20];//
//	float        volume_5days;
//	float        waipan;
//	float        liangbi;
...
typedef struct{
	...
	float zongzhi_zhd_sh;///////////    /100
	float A_guzhd_sh;///////////    /100
	float B_guzhd_sh;  //////////  
	float zhd_sh30;  //////////  
