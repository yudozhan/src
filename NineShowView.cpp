...
CNineShowView::~CNineShowView()
{
   ...
   m_pDoc->m_SystemInitData.m_TotalNineViewData.NineInitDone=INITDATAID;
	...
	for(int i=0;i<9;i++)
	{
		...
		{
			GlobalUnlock((HGLOBAL)m_NineShowData[i].pNineData);        //释放内存
...
BOOL CNineShowView::InitGridContent(CNineGridCtrl *pGrid )  //初始化Grid
{
	m_nCols=4;
    m_nFixRows=1;
	m_nFixCols=0;
...
void CNineShowView::OnStockTypeChoose(UINT nID)
{
	...
	CString str;
 	switch(which)
	{
	case 0 :  str.Format("上证A股");
		l_nStockType=1;
		break;
	case 1 :  str.Format("上证B股");
		l_nStockType=2;
		break;
	case 2 :  str.Format("深证A股");
		l_nStockType=12;
		break;
	case 3 :  str.Format("深证B股");
		l_nStockType=13;
		break;
	case 4 :  str.Format("上证债券");
		l_nStockType=4;
		break;
	case 5 :  str.Format("上证转债");
		l_nStockType=5;
		break;
	case 6 :  str.Format("上证回购");
		l_nStockType=6;
		break;
	case 7 :  str.Format("上证ETF");
		l_nStockType=7;
		break;
	case 8 :  str.Format("上证开放基金");
		l_nStockType=8;
		break;
	case 9 :  str.Format("上证权证");
		l_nStockType=9;
		break;
	case 10 :  str.Format("上证其它");
		l_nStockType=10;
		break;
	case 11 :  str.Format("深证债券");
		l_nStockType=15;
		break;
	case 12 :  str.Format("深证转债");
		l_nStockType=16;
		break;
	case 13 :  str.Format("深证回购");
		l_nStockType=17;
		break;
	case 14 :  str.Format("深证基金");
		l_nStockType=14;
		break;
	case 15 :  str.Format("深证ETF");
		l_nStockType=18;
		break;
	case 16 :  str.Format("深证开放基金");
		l_nStockType=19;
		break;
	case 17 :  str.Format("深证权证");
		l_nStockType=20;
		break;
	case 18 :  str.Format("三板行情");
		l_nStockType=21;
		break;
	case 19 :  str.Format("深证其它");
		l_nStockType=22;
		break;
	case 20 :  str.Format("中小企业");
		l_nStockType=23;
	case 21 :  str.Format("创业板");
		l_nStockType=24;
		break;
...
LRESULT CNineShowView::OnKeyboardGenius(WPARAM wPara, LPARAM lPara)
{
 	...
 	switch(m_nSharesType)
	{
		 ...
		 case 19 :  str.Format("深证其它");
				 l_nStockType=22;
				 break;
		 case 20 :  str.Format("板块指数");
				 l_nStockType=25;
				 break;
		 case 21 :  str.Format("二板指数");
				 l_nStockType=23;
				 break;
         ...
         if(strcmp(m_findzqdm.GetBuffer(0),m_NineShowData[m_nFocusCount].pNineData[i].pItem->id)==0)
			{
               ...
               m_pGridCtrl[m_nFocusCount]->Invalidate();                            //更新显示
			   l_isFind=TRUE;
			   break;
			}
	   }
         ...
         if(strcmp(m_findzqdm.GetBuffer(0),m_NineShowData[nShowCounts].pNineData[i].pItem->id)==0)
			{
               ...
               m_pGridCtrl[nShowCounts]->Invalidate();                            //更新显示
			   l_isFind=TRUE;
			   break;
			}
	   }
