...
// Attributes
public:
	float m_fRedGreen_sh; //上海红绿军
	float m_fRedGreen_sz;
	float m_fRedGreen_cy;

	float m_fIndex_sh;  //上证指数
	float m_fIndex_sz;
	float m_fIndex_cy;
	...
	float m_fVol_sh; //上海成交量
	float m_fVol_sz;
	float m_fVol_cy;

	float m_fAmount_sh;  //上海成交额
	float m_fAmount_sz;
	float m_fAmount_cy;

	BOOL m_aRcvCount[4];//实时行情、补分时、补日线、补基本资料
	int nCount;
	CRect m_aPaneRect[8];
	COLORREF bColor[10];
