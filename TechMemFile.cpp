// CTechMemFile.cpp : implementation file
//
...
//IMPLEMENT_DYNCREATE(CTechMemFile, CObject)

//sunquan begin
CTechMemFile::CTechMemFile()
{
	m_bShareMem = false;
	m_sNameShareMem = "";
...
CTechMemFile::CTechMemFile(LPCTSTR lpszFileName, UINT nOpenFlags)
{
...
int CTechMemFile::GetStockNumber()//get the total count of stock
{
...
void CTechMemFile::SetStockNumber(int nStock)//set the total count of stock
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
void CTechMemFile::SetSmallBlockCount(int nBlock)//set the total count of small block
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
int CTechMemFile::GetSmallBlockCount()//get the total count of small block
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
BOOL CTechMemFile::Open(LPCTSTR lpszFileName, UINT nOpenFlags,int nAddToFileEnd,
	CFileException* pException)
{
	...
	// CTechMemFile objects are always binary and CreateFile does not need flag
...
BOOL CTechMemFile::OpenShare(LPCTSTR lpszFileName, UINT nOpenFlags,int nAddToFileEnd,CString sShareName,
	CFileException* pException)
{
	m_bShareMem = true;
...
LONG CTechMemFile::Seek(LONG lOff, UINT nFrom)
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
DWORD CTechMemFile::GetLength() const
{
	DWORD dwRtn = m_lpvFileEnd - m_lpvFileBegin;
	return dwRtn;
}

UINT CTechMemFile::Read(void* lpBuf, UINT nCount)
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
void CTechMemFile::Write(const void* lpBuf, UINT nCount)
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
void CTechMemFile::Flush()
{
	;
}
void CTechMemFile::Close()
{
...
bool CTechMemFile::ReMap(int nAdd)
{
	if(	m_bShareMem == true)
		return true;
...
BYTE * CTechMemFile::GetFileBeginPointer()
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
BYTE* CTechMemFile::GetFileCurrentPointer()
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
void CTechMemFile::ReMapFromBegin(int nCount)
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
WORD CTechMemFile::GetMaxNumStock()
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
...
void CTechMemFile::SetMaxNumStock(WORD nMaxNumStock)
{
	if (m_bShareMem == false && m_hFile == (UINT)hFileNull ||  m_hFileMap==NULL )
	{
		ASSERT(FALSE);
