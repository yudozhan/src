...
/*	for(int i=0;i<4;i++)
	{
        if(nBig[i]<0||nBigMax[i]<0)
		{
			AfxMessageBox("选取手数时,输入参数不能为负数");
			return ;
		}
	}
	for( i=0;i<4;i++)
	{
        if(nBig[i]>=nBigMax[i]||nBigGB[i]>=nBigMaxGB[i])
		{
	...
	for(i=0;i<8;i++)
	{
		if(nParam[i]>=nParam[i+1])
		{
			AfxMessageBox("在选取统计分类手数时,前输入参数不能大于后输入参数!");
			return ;
		}
	}
	for(i=0;i<4;i++)
	{
		if(nBigGB[i]<0||nBigGB[i]>1)
		{
	...
	for(i=0;i<4;i++)
	{
		if(nBigMaxGB[i]<0||nBigMaxGB[i]>1)
		{
			AfxMessageBox("选取股本时,输入的参数不能大于1或小于0!");
			return ;
		}
	...
	for(i=0;i<8;i++)
	{
		if(nParamGB[i]>=nParamGB[i+1])
		{
			AfxMessageBox("在选取统计分类股本时,前输入参数不能大于后输入参数!");
			return ;
		}
	}
*/	CDialog::OnOK();
