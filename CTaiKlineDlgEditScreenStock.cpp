...
//  m_inputcor.SetRedraw(FALSE);
	...
	for(int flag=0;flag<1;flag++)
	{
		...
		if(n==0)
		{//把参数信息(1-4)从m_edit中恢复到m_editParam
			for(i=0;i<4;i++)
				for(j=0;j<4;j++)
					m_editParam[i][j]=GetStringParam(i,j);//m_edit[i][j];
		}
		else //把参数信息(5-8)从m_edit中恢复到m_editParam
		{
			for(i=0;i<4;i++)
				for(j=0;j<4;j++)
  ...
  if(iskline<2)
  {
	  if(equation->m_RlineNum!=1)
	  {
		  if(equation->m_nErrPos==-1)
		  {
			  ...
			  {
				  equation->m_strErrMsg="公式必须有一个返回值";
			  ...
			  else
			  {
				  ...
				  if(equation->m_nErrPos==-1&&k<-1)
				  {
					  ...
					  equation->m_strErrMsg="公式只能有一个表达式";
...
void CTaiKlineDlgEditScreenStock::ChangePeriodParamData(bool bSave)//保存、读取周期的参数数据
{
