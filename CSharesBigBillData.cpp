...
{
    m_hFile=NULL;
	m_hFileMap=NULL;                    //内存映射文件句柄
	m_pBigBillHead=NULL;                   //内存映射文件头部结构  
	m_pbData=NULL;                      //内存映射文件首部指针
	m_pMapData=NULL;         
	InitRealTimeHsData();
}
...
{
	 long l_nFileLength=sizeof(BIG_LISTHEAD)+sizeof(BIG_LIST)*5000+200 ;	
    ...
    m_pBigBillHead=(BIG_LISTHEAD *)m_pbData;                    //内存映射文件头部结构  
...
BOOL CSharesBigBillData::ClearRealTimeHsData()
{
    m_pBigBillHead->ItemCount=0;
...
BOOL CSharesBigBillData::InitRealTimeHsDataExist()
{
	 m_hFile=CreateFile("RealTimeHs.dat",GENERIC_READ|GENERIC_WRITE,
		FILE_SHARE_READ|FILE_SHARE_WRITE,
		NULL,
...
BOOL CSharesBigBillData::CalcBigBillBigData(CReportData *pCdat,RCV_REPORT_STRUCTEx* m_GpHqReport)
{
	...
	if(m_GpHqReport->m_fNewPrice >= pCdat->psel1) //大单买入
	{    
	...
	}else if(m_GpHqReport->m_fNewPrice <=pCdat->pbuy1) //大单卖出
	{
...
BOOL CSharesBigBillData::CalcBigBillBigDataGB(CReportData *pCdat,RCV_REPORT_STRUCTEx* m_GpHqReport,float GB)
{
	...
	if(m_GpHqReport->m_fNewPrice >= pCdat->pbuy1) //大单买入
	{    
	...
	}else if(m_GpHqReport->m_fNewPrice <=pCdat->psel1) //大单卖出
	{
...
void CSharesBigBillData::CalcBigBillBigHistoryDataGB(CReportData *pCdat,TRADE_DETAIL_BIG_LIST_PER *pBigBillData2,TRADE_DETAIL_BIG_LIST_PER *pBigBillData1,float GB)
{
	...
	if(pBigBillData2->price >= pBigBillData1->buyPrice[0]) //大单买入
	{    
		if((temp>=m_pBigBillHead->BigSetGB[0]*GB&&temp<=m_pBigBillHead->BigMaxSetGB[0]*GB)&&m_pBigBillHead->IsCalc[0])
		{
           BIG_LIST *pCjmx;
	...
	}else if(pBigBillData2->price <=pBigBillData1->sellPrice[0]) //大单卖出
	{
		if((temp>=m_pBigBillHead->BigSetGB[1]*GB&&temp<=m_pBigBillHead->BigMaxSetGB[1]*GB)&&m_pBigBillHead->IsCalc[1])
		{
           BIG_LIST *pCjmx;
