...
CStatusBar_sjl::CStatusBar_sjl()
{
	...
	m_fAmount_sh	=	0;  //上海成交额
	m_fAmount_sz	=	0;
	m_fAmount_cy	=	0;//亿元

	for(i=0;i<4;i++)
		m_aRcvCount[i] =FALSE;//实时行情、补分时、补日线、补基本资料
...
/*	CString strInfo;
	if( m_fAD_sz>0.0)
		strInfo.Format("深%.2f +%.2f  %.1f亿",m_fIndex_sz,m_fAD_sz,//m_fVol_sz,
		m_fAmount_sz);
	else if( m_fAD_sz == 0.0)
	{
		strInfo.Format("深%.2f 0  %.1f亿",m_fIndex_sz,
		m_fAmount_sz);
	}
	else
	{
		strInfo.Format("深%.2f -%.2f  %.1f亿",m_fIndex_sz,fabs(m_fAD_sz),//m_fVol_sz,
		m_fAmount_sz);
	}
	dc.DrawText(strInfo,&rect,DT_LEFT|DT_VCENTER|DT_SINGLELINE);*/
	CString strInfo;
...
/*	dc.Draw3dRect(&rect,GetSysColor(COLOR_BTNSHADOW),GetSysColor(COLOR_BTNHILIGHT));
	...
	CString strInfo;
	if( m_fAD_cy>0.0)
		strInfo.Format("创指%.2f 涨%.1f点 %.1f亿元",m_fIndex_cy,m_fAD_cy,//m_fVol_cy,
		m_fAmount_cy);
	else
	{
		strInfo.Format("创指%.2f 跌%.1f点 %.1f亿元",m_fIndex_cy,fabs(m_fAD_cy),//m_fVol_cy,
		m_fAmount_cy);
	}
	dc.DrawText(strInfo,&rect,DT_LEFT|DT_VCENTER|DT_SINGLELINE);
*/
