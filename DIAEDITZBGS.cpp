...
BOOL CDialogEDITZBGS::OnInitDialog() 
{
	...
	ScreenToClient(&datarect);   //定义输出的公式测试信息的位置
    datarect.left=235;
	datarect.top=205;
	datarect.right=400;
	datarect.bottom=220;

	if(item!=-1)            //如果不是新增的指标公式
	{
...
//----------------------------将坐标线位置写入结构
	...
	for( i=0;i<m_xline.GetLength();i++)
  {
	  ...
	  bool isreturn=false;             //是否是回车符的标志
	  bool isfinish=(test==';');       //是否是；符的标志
...
//------------------------将XLINE的输入内容换成合法的格式
  ...
  for(int i=0;i<m_xline.GetLength();i++)
  {
	  ...
	  if(!(isdot||isreturn||isfinish||isdot||(test>='0'&&test<='9')))
	  {
		  ...
		  errorposition=3000+i;       //XLINE错误的错误号+3000
		  errpram=true;
		  break;
	  }
	  if(isfinish||isreturn||i==m_xline.GetLength()-1)
...
void CDialogEDITZBGS::OnDyhs()    //"调" "用" 系统定义的"函" "数"
{
	UpdateData();
	...
	if(m_diazbgshs.DoModal()==IDOK)
	{
		...
		for(int i=0;i<length;i++)  //将函数名和函数的说明分开
		{
		...
		if(i==length)
			...
			functionstr=functionstr.Left(i); //只用函数的名字
...
void CDialogEDITZBGS::OnHfqs()     //"恢" "复" 公式的 "缺" "省"形式
{
	OnInitDialog();
...
//////////////////////////////////////////////////////////////////////|
//|   函数功能：主要用来实现初试化时浮点型数据向字符型的转换        //|
//|   入口参数：字符串数据的引用和浮点型的数据                      //|
//|   出口参数：无                                                  //|
//|   引用资源：无                                                  //|
///////////////////////////////////////////////////////////////////////
