...
/////////////////////////////////////////////////////////////////////////////
// CDialogEDITZJXT message handlers
...
for(j=0;j<1;j++)
{
 if(m_edit11==""&&(m_edit14!=""||m_edit15!=""))
  ...
  for(int i=0;i<m_input.GetLength();i++)    //将回车符号替换成空格
 {  
	  
	  if(m_input.GetAt(i)==13&&m_input.GetAt(i+1)==10)  //判断是否是回车符
	  { 
		 m_input.SetAt(i,' ');
		 m_input.SetAt(i+1,' ');
	  }
  }
  int inputlength=m_input.GetLength();  //将买进条件和卖出条件加入
...
void CDialogEDITZJXT::OnDyhs()    // "调" "用" "函" "数"
{
...
BOOL CDialogEDITZJXT::OnInitDialog() 
{
	...
	ScreenToClient(&datarect);   //定义输出的错误信息的位置
    datarect.left=250;
	datarect.top=50;
	datarect.right=380;
	datarect.bottom=80;
