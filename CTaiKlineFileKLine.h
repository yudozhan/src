...
typedef struct _KLINE_SMALL_HEAD2
{
	...
	int    numKln	;	  //4个字节整型，代表K线的个数。
//	WORD    numKlnExt	;	  //2个字节整型，代表扩展日K线的个数。
...
public:
	...
	static void ZeroKlineData(CString symbol,int stkKind,bool bDay );//= true
...
//	CTaiShanDoc * pDoc;
	...
	int ReadKLine(CString symbol,Kline*& pKline,int nRead=-1,int nAddBlank = 0);	//读取K线数据
	...
	bool WriteKLine(CString symbol,Kline* pKline,int nWrite,int nType);	//写K线数据
	bool WriteKlinePeriod(CString symbol, Kline *pKline,int nKline, CTime& timeStart, CTime& timeEnd);//写一段时间的K线数据
	void InitMapOfDoc();//initiate the related data in document
	void CreateSmallBlock();//when a new file created, create the small block. file length is 16+64*4096

//	void SetKLineCountPer(int nPos,int nKline);//set the total count of small block
//	int GetKLineCountPer(int nPos);//get the total count of small block
	static int RegularTime(int time,bool bDay);
	int GetKlineSmallHeader(CString symbol,KLINE_SMALLHEAD* pKlineSmallHead);//get the small header pointer of a stock
...
private:
	...
	int m_kindKline;//the kind of kline
