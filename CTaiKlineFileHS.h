...
struct HSSMALLHEAD//size 64
{
	...
	int    numHS;			//4个字节整型，代表个数。
	int    numReserved;		//4个字节整型。
	WORD   symBlock[16];	//下面每两个字节代表K线数据的块号
};
class	CTaiShanDoc;
...
public:
	...
	int ReadHS(CString symbol,CBuySellList& buySellList,bool bClearAll = true);	//读取数据
	int ReadHSPeriod(CString symbol, CBuySellList& buySellList, CTime timeStart, CTime timeEnd,bool bClearAll = true);//读取一段时间的K线数据
//	bool WriteHS(CString symbol);	//写数据 
	bool WriteHS(CReportData* pCdat,bool bFirstOne = false);	//写K线数据 CReportData
