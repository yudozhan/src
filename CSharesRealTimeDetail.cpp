...
BOOL CSharesRealTimeDetail::OnInitDialog() 
{
	...
	int i;
	for(i=0;i<m_pGrid->GetColumnCount();i++)
	{
		if(m_IsF1)
...
void CSharesRealTimeDetail::InitGridColumnTitle()
{
	if(m_IsF1)
 	  m_nRows = 250;
	else
...
int CSharesRealTimeDetail::InitGridCtrl()   	//初始化GridCtrl
{
	InitGridColumnTitle();
	InitGridContent();
	int m_Width=0;
...
BOOL CSharesRealTimeDetail::InitGridContent()  //初始化Grid
{
    m_pGrid->SetMainDocument(m_pDoc);
	try {
