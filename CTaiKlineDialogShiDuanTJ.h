...
// Construction
private:
	int m_nTimeStyle;	//用于显示时间格式 0:表示只显示日期，1：只显示时间
public:
	int m_nKlineKind2;
	CTaiShanKlineShowView* pView;
	void OnDTNotify(NMHDR *pHdr, LRESULT *pRes);//日期控件时间更改的通知消息
	void OnDTNotify2(NMHDR *pHdr, LRESULT *pRes);
	void CreateTimeCtl();
	CDateTimeCtrl m_DateTime;//开始时间日期控件
	CDateTimeCtrl m_DateTime2;//结束时间日期控件
	int m_foot;
	CTime m_tmB;
	CTime m_tmE;
	CString m_begin;
