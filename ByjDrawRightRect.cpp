...
//////////////////////////////////////////////////////////////////////
CString ByjDrawRightRect::m_nameIndex="";//LL33
...
{
	pDoc = bsDoc;		//CTaiShanDoc的指针
 	pView = bsView;		//CTaiShanKlineShowView的指针
	if(m_nameIndex == "")
	{
		GetSetReg reg;
		char ch[512];
	...
	m_nValidIndicator = 0;//指标在当前周期是否有效
...
//		int j=0;
		...
		if(ComputeFormu2(m_formuCompute,input)==0)
		{
			...
			for(i=0 ;i<m_dataFormular .numLine ;i++)
			{
...
//	pDoK->SetRectCurrent(0);
...

	{
		...
		int i;
		for(i = nFirstFoot;i<ARRAY_Right_Box::m_nCount ;i++)
		{
...
//					break;
				...
				case CROSSDOT:
					pDC->MoveTo(right-1,y-1);
					pDC->LineTo(right+1,y+1);
					pDC->MoveTo(right-1,y+1);
					pDC->LineTo(right+1,y-1);
					break;
				case BUY_CASE://
