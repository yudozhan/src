...
void CRiskIndex::GetCovMat(double *pPriceMat,int nDay,int nStock,double * &pCovMat)
{
	...
	double * tmpVec=new double[nStock];//tmpVec为 X(t)-meanX
...
double CRiskIndex::GetMaxEigVal(double *pCovMat,int nDay, int nStock)
{
	...
	//每只股票风险贡献的权重
	double dSumAbsEigVec=0.0;//单位化之用
...
{
	//如果一个股票的价格为0的个数超过nCriterion，则把该股票的价格全部变为0
	...
	for(int i=0;i<nStock;i++)
	{
		if(*(AssistVector+i)>nCriterion)
		{
			for(int j=0;j<nDay;j++)
				*(pPriceMat+j*nStock+i)=0.0;//当无效数据过多时，不计算此股票
		}
...
*/
	...
	else 
	{
		...
		CString szWeek;
		GetDlgItem(IDC_WEEKS)->GetWindowText(szWeek);//第?个样本区间
...
BOOL CRiskIndex::OnInitDialog() 
{
	...
	pListCtrl->GetClientRect(&rect);//得到CListCtrl的宽度，来计算相应的宽度.
...
double CRiskIndex::GetRiskIndex(CTime& nStart, CTime& nEnd)
{
	...
	return dMarketStd / sqrt(dMaxEigVal);	//市场风险指数=实际市场风险/最大市场风险(均为标准差)
