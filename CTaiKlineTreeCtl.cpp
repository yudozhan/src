...
/*	if(nKind == 2)
		for(int j = 0;j<nSubKind;j++)
			CFormularContent::m_strArrayKind[nKind][j] = "01 K线组合公式";
*/
	//insert often,ALL
	...
	if(nInsertKind >0 )
	{
		HTREEITEM hSub = hChapter;//InsertItem("常用公式",16,16,hChapter);
...
/*	if(-nKindRoot-1 == 0 )//add to 所有技术指标 group
	{//m_hAllIndex
...
HTREEITEM CTaiKlineTreeCtl::FindGroupQiTa(HTREEITEM hRoot,CString sName)// "其它"
{
...
void CTaiKlineTreeCtl::OnRclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(nKindItem == 0 || pIndex==NULL)
		if(sNameItem == g_chAllIndex )//|| sNameItem == "其它" 
			return;
