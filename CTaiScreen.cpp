...
BOOL CTaiScreen::OnInitDialog() 
{
	...
	AddCComboBoxString(*((CComboBox*)GetDlgItem(IDC_AGGREGATION1)));//IDC_AGGREGATION1
...
{
	*pResult = 0;
}  //OnClickStockList(NMHDR* pNMHDR, LRESULT* pResult) 
...
void CTaiScreen::OnChoosestock() 
{
	...
	m_resultarray.RemoveAll ();//清空结果数组
    Kline *pKline=NULL;
	...
	for(i=0;i<index_num;i++)
	{
		...
		int nDays=index_save.iBeginFoot ;//GetCountValid(&index_save,pDoc,10);
		pnNeedCount[i]=nDays;
	}
	...
	m_bExecuteCalc = true;
	EnableControl(false);
    for(i=0;i<stocknum && m_bExecuteCalc == true;i++)//计算每一个股票
    {
		...
		if(CMainFrame::m_pWndTJXG == NULL) return;
...
int CTaiScreen::FindStr(SymbolKindArr* array,SymbolKind& str)
{
  ...
  return -1;
} //int CTaiScreen::FindStr(CStringArray* array,CString str)
...
void CTaiScreen::OnButton1() //是否显示集合运算
{
...
bool CTaiScreen::CalcEach(IndexDataInfo& index_save2,CString StockId, int stkKind, int nCount, Kline *pKline, CTime &tmB, CTime &tmE, CBuySellList *pList)
{
	...
	if(index_num>0)
	{
		...
		index_save = index_save2;//return false;

	int nn = index_num;
	if(nn ==0) nn=1;
	for (int k=0;k<nn;k++)//计算每个指标
	{
		if(index_num)
			index_save=m_indexSaveArray.GetAt(k);
