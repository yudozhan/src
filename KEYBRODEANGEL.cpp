...
BOOL CTaiKeyBoardAngelDlg::OnInitDialog() 
{
    ...
    //First get the handle of the public Doc ---- Data Center.
...
//
//This function calculate the item range that will be showed in the list control
//
//Input parameter:
//      strInput: current input string by the user
//
void CTaiKeyBoardAngelDlg::CalculateListRange(CString strInput)
{
	//
	//First get the active docment handle
	//then get the data pointer
	//
//	CMDIChildWnd* pchildfram= ((CMainFrame*)(AfxGetApp()->m_pMainWnd))->MDIGetActive();
//	CTaiShanDoc*   pDoc=(CTaiShanDoc*)pchildfram->GetActiveDocument();
    CTaiShanDoc*	pDoc=CMainFrame::m_pDoc ;//(CTaiShanDoc*)pchildfram->GetActiveDocument();
...
//
//This function insert an item to the listctrl
//
//Three Input parameters:
//      nWhichItem : which item will be inserted to the list control
//      nStartItem : the index of start item in the data center of course also in the list control
//      nDefItem   : index of the default show item 
//
	...
	switch(pMsg->message)
	{
		case(WM_KEYDOWN):
			...
			switch(pMsg->wParam)
			{
				...
				case(13):
					...
					break;
				/*
					i=m_nCurrentItemIndex+m_nStartPosArray.GetAt(nLength)+m_nListPosition;
		...
		case(WM_CHAR):
			if(m_bLastFocusFlag&&((pMsg->wParam>='0'&&pMsg->wParam<='9')||pMsg->wParam>='A'&&pMsg->wParam<='Z'||pMsg->wParam>='a'&&pMsg->wParam<='z'))
			{
				m_InputEditCtrl.SetFocus();
				m_bLastFocusFlag=FALSE;
				m_strInputResult = m_strInputResult + (const char)pMsg->wParam;
...
LRESULT CTaiKeyBoardAngelDlg::OnDragBegin(WPARAM wParam, LPARAM lParam)
{
	if(wParam == 0)
	{
	    ...
	    this->PostMessage(WM_CLOSE);//this->OnCancel();
