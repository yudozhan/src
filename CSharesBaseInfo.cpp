...
//	#define MAXSTOCKBASEBUFFER 40+2000*(160+2240)
	...
	m_pBaseInfo=(BASEINFO *)temp;		//股票行情数据结构
...
{
    HANDLE hFileBase,hFileMapBase;                  //内存映射文件句柄
	DWORD nlength=0;
	BYTE  *pbDataBase;                          //内存映射文件首部指针
	char  *pFileData;
	char ls_path[255];
	char temp[100];
...
BOOL CSharesBaseInfo::SetMemoryAllocSize(unsigned int nSize)
{
	...
	else
	{
		...
		GlobalUnlock((HGLOBAL)m_pBaseInfoPoint);        //释放内存
...
BOOL CSharesBaseInfo::AddStockItem(char *pStockCode,int nKind,PBASEINFO & pBaseItem)
{
	...
	if(m_BaseFileHead->StockCount + 1 >m_BaseFileHead->MaxStockCount)  //检验现有股票数是否大于最大保存数
	{
		AddBlockDataSize();
	}
...
BOOL CSharesBaseInfo::InsertItemPoint(BASEINFO *m_pStk)               //插入一个股票
{
